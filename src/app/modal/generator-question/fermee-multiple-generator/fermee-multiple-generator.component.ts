...
@Component({
  selector: 'app-fermee-multiple-generator',
  standalone: true,
  ...
  templateUrl: './fermee-multiple-generator.component.html',
  styleUrls: ['./fermee-multiple-generator.component.scss']
})
export class FermeeMultipleGeneratorComponent {
  ...
  genererQuestion(){
    this.spin=true;
    this.api.getQuestionFromLmstudio().subscribe((response) => {
      this.libelle_question = response.content;
    });
    this.spin=false;
  }
  genererModalites(){
    this.api.getModalitesFromLmstudio(this.libelle_question).subscribe((response) => {
