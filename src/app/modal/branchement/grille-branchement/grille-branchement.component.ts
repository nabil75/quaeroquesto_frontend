...
@Component({
  ...
  standalone: true,
  imports: [MatFormFieldModule, MatInputModule, FormsModule, NgFor, AutosizeModule, MatRadioModule],
  templateUrl: './grille-branchement.component.html',
  styleUrls: ['./grille-branchement.component.scss']
})
export class GrilleBranchementComponent {
  ...
  selectedColonne: string[] = [];
  selectedDestination: string = '';
  selectedCondition: number = -1;
  conditions: string[] = ["Si les réponses sont ...", "Si les réponses sont différentes de ...", "Si les réponses sont parmi les suivantes :",
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    if (this.branchementsModal && this.lignes && this.colonnes && this.questions) {
  ...
  addBranchement() {
    if (this.selectedCondition != -1) {
      if (this.selectedCondition == 0 || this.selectedCondition == 1 || this.selectedCondition == 2) {
        if (this.selectedLigne.length && this.selectedColonne.length && this.selectedDestination.length) {
          const n = this.branchementsModal.length;
          const sourceLignes = this.selectedLigne;
          const indexSourcesToAdd: any[] =[]
          sourceLignes.forEach(source =>{
            indexSourcesToAdd.push( this.lignes.findIndex((ligne: { libelle: string; }) => ligne.libelle === source))
          });
          ...
          let i =1;
          sourceLignes.forEach(source =>{
            const indexSourcesToDisplay = this.lignes.findIndex((ligne: { libelle: string; }) => ligne.libelle === source);
            sourcesToAdd.push("("+(i)+") "+this.lignes[indexSourcesToDisplay].libelle+" ");
            i++;
          });
    ...
    this.branchementsModal.forEach(branchement => {
      ...
      if (branchement.source.length>0) {
        ...
        branchement.source.forEach((sourceModal: any) =>{
          sourceModal = this.lignes[sourceModal].libelle;
  ...
  onClickOnLigne(event: any){
    // let source = event.target.getAttribute('value');
    // this.selectedLigne.push(source)
    console.log(this.selectedLigne)
  }

  onClickOnColonne(event: any){
    // let source = event.target.getAttribute('value');
    // this.selectedColonne.push(source)
    console.log(this.selectedColonne)
  }
}